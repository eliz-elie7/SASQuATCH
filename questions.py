...
def list_questions_by_pseudonym(
    ...
    sur un pseudonyme dans le visualiseur.
    """
    course_session = _get_owned_session_or_error(db, session_id, current_teacher)
...
# ── Modèle Pydantic pour le body du cluster ──────────────────────────────────
class ClusterRequest(BaseModel):
    questions: List[Any]


@router.post("/cluster")
async def cluster_questions(payload: ClusterRequest, themes_count: int = 3):
    """
    Reçoit le tableau des questions d'une session et renvoie les groupes
    triés sémantiquement par l'IA (§2.4.2).
    """
    try:
        resultat = regrouper_questions_par_theme(payload.questions, n_clusters=themes_count)
        return resultat
